...
export default function ViewTeachers() {
  ...
  const callAboutPage = async () => {
    try {
      const response = await axios.get("http://localhost:5000/viewteachers", {
  ...
  return (
    ...
    <div className="whole">
        <div className="addButton">
            <Link to="/teachers/add"><button>Add New</button></Link>
        </div>
      <div className="main">
        <div class="h2 font-weight-bold">Teachers</div>
              ...
              {studentsData.length > 0 ? (
                studentsData.map((user) => (
                  <tr  class="my" key={user._id}>
                    ...
                    <td>
                      <Link class=" mr-2" to={`/teachers/edit/${user._id}`}>
